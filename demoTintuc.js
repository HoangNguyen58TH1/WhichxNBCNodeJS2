...
(async () => {
    ...
    for (let url of urlArray) {
        ...
        const articles3 = await page.evaluate(() => {
            ...
            let urlTintuc = document.querySelectorAll('ul.list-news-content > li.news-item > div.name-news > p.sapo');
            //thethao
            // let urlTintuc = document.querySelectorAll('ul.list-news-content > li.news-item > div.txt > h3 > a');
            // urlTintuc = [...urlTintuc].slice(4,15);
            urlTintuc = [...urlTintuc].slice(0, 15);
        ...
        element = articles3.map(e => {
            handlingStrings = wordTokenizer.tag(removePunctuation(e.description).replace('-', ''), 'text').toLowerCase().split(' ').filter(val => !stopwords.includes(val)).join(' ')
        ...
        for (let i = 0; i < element.length; i++) {
            var News = category.classify(element[i]);
            ...
            idtheloaiArray.push(idtheloai)

        }
        console.log('idtheloaiArray:', idtheloaiArray);

        //handling files
        //   let totalArray = articlesArray.map(e => e.idLoaiTin)
        fs.appendFile('filenew.js', 'arrayNews' + indexUrl + ' = [' + idtheloaiArray + ']\n', function (err) {
            if (err) throw err;
            console.log('Save');
        })
